...
def get_frame_number(image_name):
    frame_num = int(image_name.split('.')[0].split('_')[-1]) - 1
    return frame_num

def load_all_radar_info(annotations):
    radar = []
    maps_set = set()
    maps_list = []
...
def image_name_to_map_file(image_name):
    ...
    trip = '-'.join(image_name.split('-')[:4])
    map_file = '_'.join(image_name.split('split_0_')[-1].split('.')[0].split('_')[:-1])[:-1] + '.map'
    full_map_file = '/'.join([q50_dir, trip, map_file])
    return full_map_file

def copy_from_turked(annotations_turked, annotations_tracked):
    ind = 0
    for antu in annotations_turked:
        while antu.imageName != annotations_tracked[ind].imageName:
            ind += 1
        annotations_tracked[ind] = antu
    # mapframe2inds = {}
    # name2key = lambda name : (os.path.basename(image_name_to_map_file(name)), get_frame_number(name))
    # for ind,antr in enumerate(annotations_tracked):
    #     key = name2key(antr.imageName)
    #     if key in mapframe2inds:
    #         mapframe2inds[key].append(ind)
    #     else:
    #         mapframe2inds[key] = [ind]


    # insert_pairs = []
    # for antu in annotations_turked:
    #     key = name2key(antu.imageName)
    #     if key in mapframe2inds:
    #         for ind in mapframe2inds[key]:
    #             annotations_tracked[ind] = antu
    #     else:
    #         key = key[0],key[1]-1
    #         assert key in mapframe2inds
    #         ind = mapframe2inds[key]
    #         insert_pairs.append( (ind, antu) )

    # # now insert pairs
    # print 'done'

def copy_from_tracked(annotations_turked, annotations_tracked):
    ind_tracked = 0
    for ind_turked, antu in enumerate(annotations_turked):
        while antu.imageName != annotations_tracked[ind_tracked].imageName:
            ind_tracked += 1
...
def main(filename=None):
    ...
    data_folder = os.path.dirname(image_name_to_map_file(annotations_tracked[0].imageName))
    args = {'cam_num': 2, 'params':LoadParameters(open(data_folder + '/params.ini', 'r').readline().rstrip())}
    args['video'] = image_name_to_map_file(annotations_tracked[0].imageName).replace('.map', '2.avi')
    params = args['params']
